...
public final class SimdInterpreterMachine extends InterpreterMachine {
    ...
    protected void evalDefault(
        ...
        switch (instruction.opcode()) {
            ...
            case OpCode.I64x2_ABS:
                UNARY(stack, LongVector::reinterpretAsLongs, Vector::abs);
                break;
            case OpCode.I64x2_NEG:
                UNARY(stack, LongVector::reinterpretAsLongs, Vector::neg);
                break;
            ...
            case OpCode.F32x4_EQ:
                F32x4(stack, (a, b) -> (OpcodeImpl.F32_EQ(a, b) > 0 ? 0xFFFFFFFFL : 0x0L));
                break;
            ...
            case OpCode.F32x4_LE:
                F32x4(stack, (a, b) -> (le(b, a) ? 0xFFFFFFFFL : 0x0L));
                break;
            case OpCode.F32x4_LT:
                F32x4(stack, (a, b) -> (lt(b, a) ? 0xFFFFFFFFL : 0x0L));
                break;
            case OpCode.F32x4_GE:
                F32x4(stack, (a, b) -> (ge(b, a) ? 0xFFFFFFFFL : 0x0L));
                break;
            case OpCode.F32x4_GT:
                F32x4(stack, (a, b) -> (gt(b, a) ? 0xFFFFFFFFL : 0x0L));
                break;
            case OpCode.F32x4_NE:
                F32x4(stack, (a, b) -> (!equals(a, b) ? 0xFFFFFFFFL : 0x0L));
                break;
            ...
            case OpCode.F64x2_PMIN:
                F64x2(stack, (a, b) -> Value.doubleToLong((a < b) ? a : b));
                break;
    ...
    private static void F64x2(MStack stack, BiFunction<Double, Double, Long> fn) {
        var v1High = stack.pop();
        var v1Low = stack.pop();
        ...
        var result =
                Value.i64ToVec(
                        new long[] {
                            fn.apply(v1[0], v2[0]), fn.apply(v1[1], v2[1]),
                        });

        System.arraycopy(result, 0, stack.array(), offset, 2);
    }

    private static void F64x2(MStack stack, Function<Double, Long> fn) {
        var offset = stack.size() - 2;
    ...
    private static void I64x2_EXTMUL_LOW_I32x4_U(MStack stack) {
        ...
        var res =
                new long[] {
                    Integer.toUnsignedLong(v1[0]) * Integer.toUnsignedLong(v2[0]),
                    Integer.toUnsignedLong(v1[1]) * Integer.toUnsignedLong(v2[1]),
                };
        var result = Value.i64ToVec(res);
        System.arraycopy(result, 0, stack.array(), offset, 2);
    }
    ...
    private static void I64x2_EXTMUL_HIGH_I32x4_U(MStack stack) {
        ...
        var res =
                new long[] {
                    Integer.toUnsignedLong(v1[2]) * Integer.toUnsignedLong(v2[2]),
                    Integer.toUnsignedLong(v1[3]) * Integer.toUnsignedLong(v2[3]),
                };
